...
03_Entrenamiento_HNLP_MC_Requerimientos.ipynb (Última mod: 2026-09-19 16:52:29)
"""
...
df['texto_solicitud'] = (df['short_description'].fillna('') + ' ' + df['description'].fillna('')).apply(limpiar_cadena)

# 2. Canal de Rol / Cargo del Solicitante
df['cargo_solicitante'] = df['requested_for.title'].fillna('desconocido').apply(limpiar_cadena)

# 3. Canal de Metadatos Categóricos Discretos (Empresa)
df['empresa'] = df['requested_for.company'].fillna('DESCONOCIDO').astype(str)
...
canal_empresa = OneHotEncoder(handle_unknown='ignore', sparse_output=True)

# Ensamblar ColumnTransformer
preprocesador_hnlp = ColumnTransformer(
    transformers=[
        ('canal_1_texto', canal_texto, 'texto_solicitud'),
        ('canal_2_cargo', canal_cargo, 'cargo_solicitante'),
        ('canal_3_empresa', canal_empresa, ['empresa'])
    ],
    remainder='drop'
)
...
print(f"  -> Características Canal 1 (Texto Solicitud): {preprocesador_hnlp.named_transformers_['canal_1_texto'].get_feature_names_out().shape[0]}")
print(f"  -> Características Canal 2 (Cargo Solicitante): {preprocesador_hnlp.named_transformers_['canal_2_cargo'].get_feature_names_out().shape[0]}")
print(f"  -> Características Canal 3 (Empresa OHE): {preprocesador_hnlp.named_transformers_['canal_3_empresa'].get_feature_names_out().shape[0]}")
...
print("\n" + "="*85)
print("DESEMPEÑO EN TEST SET — ARQUITECTURA HNLP-MC (REQUERIMIENTOS)")
print("="*85)
...
ax.set_title("Comparativa de Clasificadores en Arquitectura HNLP-MC (Test Set - Requerimientos)", fontsize=12, fontweight='bold')
ax.set_ylim(0, 1.05)
for p in ax.patches:
    if p.get_height() > 0:
...
print(f"\n[MEJOR MODELO HNLP-MC]: {mejor_nombre_mc}")
...
print()
print("Para utilizar en Assigner_Requirements.py o RPA ServiceNow:")
print("  pipeline = joblib.load('semisupervised_model/pipeline_HNLP_MC.joblib')")
print("  predicciones = pipeline.predict(df_requerimientos_nuevos)")
